...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n"
              ...
              << "  --dump-frame-cache      Dump pre-encode original/masked raw frame caches under <outDir>/frame_cache\n"
              << "  --frame-hysteresis <0|1> Enable full-frame Ref/Raw hysteresis (default: 1)\n"
              << "  --frame-hysteresis-confirm <n> Consecutive frames needed to switch full-frame mode (default: 2)\n"
...
int main(int argc, char* argv[]) {
    ...
    // Record full argv for report.json reproducibility.
    {
    ...
    static struct option long_options[] = {
        ...
        {"yolo-matcher", required_argument, 0, OPT_YOLO_MATCHER},
        ...
        {"dump-frame-cache", no_argument, 0, OPT_DUMP_FRAME_CACHE},
    ...
    while ((c = getopt_long(argc, argv, "i:o:m:c:t:s:A:de:hpT:", long_options, &option_index)) != -1) {
        ...
        case 'p':
            ...
            case 'f':
                ...
                break;
            case 1040:
                opt.pixelUseKalman = (std::stoi(optarg) != 0);
                break;
            case 'x':
            ...
            case 'R':
                ...
                break;
            case OPT_YOLO_MATCHER:
                opt.yoloMatcher = optarg ? std::string(optarg) : std::string();
                break;
            case 'L':
            ...
            case 'Q':
                ...
                break;
            case OPT_YOLO_HEAL_FALLBACK_WINDOW:
                opt.yoloHealFallbackWindowFrames = std::stoi(optarg);
                break;
            case OPT_YOLO_HEAL_FALLBACK_MINSIM:
                opt.yoloHealFallbackMinSim = std::stof(optarg);
                break;
            case OPT_YOLO_HEAL_FALLBACK_MINSIM_I:
                opt.yoloHealFallbackMinSimI = std::stof(optarg);
                break;
            case OPT_YOLO_HEAL_MAX_SHIFT:
                opt.yoloHealMaxShiftPx = std::stoi(optarg);
                break;
            case OPT_YOLO_HEAL_MIN_COVERAGE:
                opt.yoloHealMinMaskCoverage = std::stof(optarg);
                break;
            case OPT_YOLO_HEAL_MSE:
                opt.yoloHealAppearanceMseThr = std::stof(optarg);
                break;
            case OPT_BUILD_INDEX:
                opt.buildIndexOnly = true;
                break;
            case OPT_LATENT_BANK:
                opt.latentBankLoadPath = optarg;
                break;
            case OPT_LATENT_NO_MINT:
                opt.latentNoMint = true;
                break;
            case OPT_GOP_SIZE:
                opt.gopSize = std::stoi(optarg);
                break;
            case OPT_ENC_GOP:
                opt.encGop = std::stoi(optarg);
                break;
            case OPT_ENC_CRF:
                opt.encCrf = std::stoi(optarg);
                break;
            case OPT_ENC_BITRATE_MBPS:
                opt.encBitrateMbps = std::max(0.0, std::stod(optarg));
                break;
            case OPT_ENC_MAXRATE_MBPS:
                opt.encMaxrateMbps = std::max(0.0, std::stod(optarg));
                break;
            case OPT_ENC_BUFSIZE_MBITS:
                opt.encBufsizeMbits = std::max(0.0, std::stod(optarg));
                break;
            case OPT_ENC_PRESET:
                opt.encPreset = optarg;
                break;
            case OPT_ENC_TUNE:
                opt.encTune = optarg;
                break;
            case OPT_ENC_PROFILE:
                opt.encProfile = optarg;
                break;
            case OPT_ENC_LEVEL:
                opt.encLevel = optarg;
                break;
            case OPT_ENC_SCENECUT:
                opt.encNoScenecut = (std::stoi(optarg) == 0);
                break;
            case OPT_ENC_AUD:
                opt.encAud = (std::stoi(optarg) != 0);
                break;
            ...
            case OPT_MASK_COLOR: {
                ...
                if (v == "class") {
                    ...
                    opt.maskColor = "rgb";
                    opt.maskColorR = clamp(r); opt.maskColorG = clamp(g); opt.maskColorB = clamp(b);
                } else {
                    opt.maskColor = "green";
            ...
            case OPT_FEATHER_PX:
                opt.featherPx = std::max(0, std::stoi(optarg));
                break;
            case OPT_FILL_BLUR_SIGMA:
                opt.fillBlurSigma = std::max(0.0f, std::stof(optarg));
                break;
            ...
            case OPT_FILL_INPAINT_RADIUS:
                opt.fillInpaintRadius = std::max(1, std::stoi(optarg));
                break;
            ...
            case 'A':
                opt.paintAlpha = std::stof(optarg);
                break;
            case OPT_DUMP_FRAME_CACHE:
                opt.dumpFrameCache = true;
                break;
            case OPT_FRAME_HYSTERESIS:
                opt.frameModeHysteresis = (std::stoi(optarg) != 0);
                break;
            case OPT_FRAME_HYSTERESIS_CONFIRM:
                opt.frameModeHysteresisConfirmFrames = std::max(1, std::stoi(optarg));
                break;
            case OPT_YOLO_FORCE_MASK_ALL:
                opt.yoloForceMaskAll = true;
                break;
            case 'N':
    ...
    if (!opt.yoloMatcher.empty()) {
        for (auto &ch : opt.yoloMatcher) ch = (char)std::tolower((unsigned char)ch);
        opt.yoloLatentKey = (opt.yoloMatcher == "latent_key");
    }
    ...
    // Always log the exact invocation so each run's parameters
    // are recoverable from run.log (stdout/stderr capture).
    std::cout << "[Command]";
    for (int i = 0; i < argc; i++) {
        std::cout << " " << argv[i];
    }
    ...
    // Echo key experiment knobs to logs so sweep issues are visible.
