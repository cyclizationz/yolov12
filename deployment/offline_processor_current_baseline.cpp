...
int run_offline_evaluation(const OfflineOptions &opt, YOLO_V8& yoloDetector) {
    ...
    if (opt.yoloLatentKey && !opt.pixelMode) {
    ...
    while (cap.read(frame)) {
        ...
        int frame_flags_i = ref_regions > 0 ? 1 : 0; // 0=raw, 1=ref
        ...
        if (opt.dumpFrameCache && originalFrameCache.is_open() && maskedFrameCache.is_open()) {
            ...
            cv::Mat processedWrite = processed.isContinuous() ? processed : processed.clone();
