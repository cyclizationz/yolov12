...
- Encoder defaults (FC5/FM6): `libx264`, `superfast`, `gop=60`, `scenecut=0`, `tune=none`, `aud=1`, `repeat-headers=1`
- Mario encoder (matched pure + RESPAWN): `libx264`, `medium`, `tune=none`, `--enc-open-gop-defaults`, fixed VBV sweep
- Current FC5 Exp1 setting is an upper-bound non-heal mode: latent-key masking without `--yolo-heal-only`
- FC5 mask profile `local_pad64_mode`: per-region local dominant (stat=mode), `mask-color-local-pad=64`, `feather-px=4` (gop_analysis full-clip winner)
- Mario pixel mode (50×44 template): `pixel-force-scale=1.0`, `pixel-max-peaks=90`, `pixel-bootstrap=15`, `pixel-flow=1`, `pixel-adaptive-thr=1`, `pixel-thr-k=0.8`, grid header/snap, band scan
