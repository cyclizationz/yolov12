...
- These rows are from the CRF23 five-clip rerun under `record/RESPAWN2026/exp4/mario_crf23_ablation/`, using the same normalized Mario clips and encoder family as Exp3/Exp5.
...
- `Artifact frames` is the mean raw number of frames with `rec_ssim < 0.95` per clip; use it as a coarse threshold diagnostic, not a final subjective-quality score.

| Mario strategy | Clips | BSP / saving | Avg recovered SSIM | Avg recovered PSNR | Artifact frames |
| --- | ---: | ---: | ---: | ---: | ---: |
...
- The full pipeline now matches the Exp5 CRF23 Mario mean (+30.2%), so the ablation is protocol-consistent with the headline Mario number.
- Removing optical flow clearly reduces BSP. `No Kalman` and `Template only` are similar or slightly higher on BSP in this table, so those variants need a quality/stability follow-up before claiming the full path is strictly better.
...
| FC5 | blur | N/A | N/A | N/A | no saved blur fill run in repo |
| FC5 | inpaint | −2.53 | 0.8989 | 33.85 | supported (CRF23 open x264 fc5_00; differs from final3 CRF18 black/dominant pair) |
